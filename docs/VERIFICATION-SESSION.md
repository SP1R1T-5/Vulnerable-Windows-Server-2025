...
BEFORE `Stage-CyberRange.ps1`, the Part B matrix goes AFTER `Setup-CyberRange.ps1`,
on a snapshotted VM.
...
| A3 | **ForceAutoLogon risk** | inspect the autologon block in `Setup-CyberRange.ps1` for `ForceAutoLogon` | If `ForceAutoLogon=1`, A2 MUST hold, or `ForceAutoLogon` is removed |
...
## PART A2 — Cross-VM gates (run on two finished participant VMs)
...
do not hand the VMs out.**

> Since 2026-09-07 each VM is built from scratch rather than cloned, so A7–A11
> should pass trivially — two independent promotions cannot share a domain SID or
> a `krbtgt`. Run them anyway on the first pair you build: they are cheap, and
> they are what proves the no-clone model delivers what the clone model was
> trying to.
...
| A13 | **Operator secrets** | compare DSRM / autologon / operator passwords on A and B | Understood and recorded off-box. They are the SAME on every VM (one config file), which is accepted: the isolation that matters comes from A7-A9, and a participant who is Domain Admin on their own box can read their own config regardless (F18) |
