...
'@
    ...
    if (-not $AllowPlaceholderCredentials) {
        ...
        $placeholders = @('Password!','S@feM0de-ChangeMe!','ChangeMe-BreakGlass!2026')
        $vals = @{}
        ...
        if ($Config.BreakGlass -and $Config.BreakGlass.ContainsKey('Password')) { $vals['BreakGlass.Password'] = [string]$Config.BreakGlass.Password }
        $bad = @()
...
function Get-RangeCloneIdentity {
    <#
        WP2 / F16 -- the SINGLE source of truth for per-clone identity and secrets.

        The golden image is cut BEFORE promotion, so every clone promotes its own
        forest and therefore gets its own domain SID and its own krbtgt for free.
        What this function supplies is the rest: a unique name, a unique domain,
        and four operator secrets derived from a master secret the instructor
        holds.

        Derivation is HMAC-SHA256(MasterSecret, "<cloneId>/<purpose>"). That means:
          * the instructor can recompute any clone's secrets from CloneId + master,
            with no bookkeeping across thirty VMs;
          * the master secret is never written to a student VM, only derived values;
          * a participant who is Administrator on their own box can read their own
            derived secrets but cannot derive any peer's.

        Both Initialize-RangeClone.ps1 (writes them) and Get-RangeCloneSecret.ps1
        (recovers them) call THIS function. Two implementations of the derivation
        would drift and silently produce clones nobody can get back into.

        Deterministic: same master + same CloneId always yields the same output.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$CloneId,
        [Parameter(Mandatory)][string]$MasterSecret,
        [string]$BaseDomain = 'range.lab'
    )

    # Normalise so -CloneId 7 and -CloneId 07 are the same clone. Getting this
    # wrong would hand out secrets that do not open the box.
    if ($CloneId -notmatch '^\d{1,2}$') {
        throw "CloneId must be 1-2 digits (got '$CloneId'). Use 01-99."
    }
    $id = '{0:D2}' -f [int]$CloneId

    function Convert-ToPassword {
        param([byte[]]$Bytes)
        # Strip non-alphanumerics so the value never needs quoting on a command
        # line or in an INF, then append a fixed complexity suffix. 24 alphanumeric
        # characters is ~143 bits -- the trim costs nothing that matters.
        $s = ([Convert]::ToBase64String($Bytes)) -replace '[^A-Za-z0-9]',''
        if ($s.Length -lt 24) { throw 'derivation produced too few usable characters' }
        $s.Substring(0,24) + 'Aa1!'
    }
    function Get-Derived {
        param([string]$Purpose)
        $h = [System.Security.Cryptography.HMACSHA256]::new([Text.Encoding]::UTF8.GetBytes($MasterSecret))
        try { Convert-ToPassword $h.ComputeHash([Text.Encoding]::UTF8.GetBytes("$id/$Purpose")) }
        finally { $h.Dispose() }
    }

    [pscustomobject]@{
        CloneId             = $id
        ComputerName        = "RANGE$id-DC01"          # 12 chars, inside the 15-char limit
        DomainName          = "r$id.$BaseDomain"
        NetbiosName         = "R$id"
        AdminPassword       = Get-Derived 'adminpw'      # becomes LocalAdminAutoLogonPass
        DsrmPassword        = Get-Derived 'dsrm'
        BreakGlassPassword  = Get-Derived 'breakglass'
        HiddenAdminPassword = Get-Derived 'hiddenadmin'
        # WP2 listed four secrets and omitted this one. It has to be derived too:
        # 'analyst' is a Domain Admin on every clone, and each clone is its own
        # forest, so a shared analyst password means a student who learns it on
        # their own box can log into every peer's DC as that peer's analyst. Same
        # cross-VM compromise F16 is about, via a different door.
        AnalystPassword     = Get-Derived 'analyst'
    }
}
...
Export-ModuleMember -Function Initialize-RangeContext, Write-RangeLog, Write-RangeManifest,
    ...
    Disable-RangeUpdateServices, Get-RangeCloneIdentity
