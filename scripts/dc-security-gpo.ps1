...
<#  DC step 6: push the machine-security downgrades INTO the Default Domain
    ...
    Controls moved into the GPO here:
      ...
      * LanmanServer EnableSecuritySignature  = 0  (server does not even offer it)
...
# F40: also clear EnableSecuritySignature ("...sign if the client agrees"). The
# DC policy otherwise re-asserts this to 1, so the server keeps OFFERING signing
# and the smb.srv.enable control reverts to FAIL after gpupdate.
Set-InfKey $sec 'Registry Values' 'MACHINE\System\CurrentControlSet\Services\LanManServer\Parameters\EnableSecuritySignature' '4,0'
