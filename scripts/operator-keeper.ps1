...
powershell -NoProfile -ExecutionPolicy Bypass -File C:\CyberRange\scripts\operator-account.ps1
...
if ($isDC) {
    ...
    $adReady = $false
    for ($i = 0; $i -lt 30; $i++) {
        # Self-heal a DISABLED service: Start-Service cannot start one, so a
        # Disabled ADWS is the classic "AD DS unreachable" trap that no amount of
        # Start-Service will fix. Force any of these back to Automatic first.
        foreach ($svc in 'NTDS','DNS','ADWS','Netlogon') { $s = Get-Service $svc -ErrorAction SilentlyContinue; if ($s -and $s.StartType -eq 'Disabled') { Set-Service $svc -StartupType Automatic -ErrorAction SilentlyContinue; L "re-enabled $svc (was Disabled)" } }
