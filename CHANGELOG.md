...
controller always OFFERS SMB signing** because SYSVOL/NETLOGON require it, and the
server service re-forces `EnableSecuritySignature=1` regardless of the registry OR
the Default Domain Controllers Policy — which is why the adjacent
`RequireSecuritySignature=0` sticks but this one never does. Two changes: (1)
`smb.srv.enable` is now a **role-aware custom control** that reports **N-A on a
DC** (not settable by any supported mechanism; same category as the F38 log-size
N-A) and still PASS/FAIL on a non-DC; (2) `scripts/dc-security-gpo.ps1` also writes
`EnableSecuritySignature=4,0` into the template as a best effort for non-DC / future
cases. Relay was never actually blocked — only *required* signing blocks it, and
that is off (PASS). To clear on a running DC, update `modules\RangeControls.psm1`
on the box and re-run `Test-RangeConfig.ps1` (no rebuild needed).

### Fixed — 2026-09-07 (F41 — AD CS / FS-SMB1 need a reboot the build never gave)
On a clean rebuild, `[dc-adcs] CertSvc running`, `[dc-adcs] ESC1 published`, and
`[live] SMB1 enabled` all FAILed. Root cause confirmed on hardware: installing the
**AD CS role leaves it `InstallState=InstallPending` / `RestartNeeded=Yes`** — the
`ADCSDeployment` module and `Install-AdcsCertificationAuthority` do not exist, and
`CertSvc` never comes up, until the box reboots. Setup configured the CA in the
SAME session, so it always failed on a clean build; earlier "passing" runs were on
a contaminated box where the feature was already fully installed. FS-SMB1 is the
same (the SMB1 server protocol is not live until a reboot completes the install).
Two changes: (1) `Stage-CyberRange.ps1` now **pre-installs `ADCS-Cert-Authority`,
`ADCS-Web-Enrollment` and `FS-SMB1` in Stage 1**, whose existing reboot completes
the pending install — so by Setup the features are settled and the CA configures
in-session (gated on `DC.InstallAdcs` / `Categories.SmbNetwork`); (2)
`scripts/ad-certificates-esc1.ps1` (F41 earlier this day) captures the feature
result and, if `ADCSDeployment` is still missing, says plainly the role needs a
reboot instead of mislabeling it. Recover on a already-built box: `Restart-Computer`,
then `.\scripts\ad-certificates-esc1.ps1` and `Set-SmbServerConfiguration -EnableSMB1Protocol $true -Force`.
**Verified on hardware 2026-09-07 (`WIN-UGR8U7OFA11`):** after the reboot + re-run,
`CertSvc` came up, ESC1 published, and SMB1 went live — the three FAILs cleared.

### Fixed — 2026-09-07 (F39 — a Disabled ADWS silently sank every AD step)
All three AD-wait loops (`Stage`, `Setup`, and `operator-keeper`) called
`Start-Service NTDS,ADWS,DNS,Netlogon -ErrorAction SilentlyContinue`. But
`Start-Service` **cannot start a service whose StartType is `Disabled`** — it
throws, the `SilentlyContinue` swallows it, and the loop times out with "AD DS
unreachable". On the field DC (`WIN-UGR8U7OFA11`) ADWS was `Disabled`, so every
directory step failed for one hidden reason: `analyst` and the five hidden domain
admins were never seeded, autologon was withheld (no `Set-ADAccountPassword`), and
the checker's 8 FAILs were all one root cause. Each loop now **re-enables any of
NTDS/DNS/ADWS/Netlogon found `Disabled` (Set-Service -StartupType Automatic)
before starting it**, so the operator-keeper self-heals a disabled ADWS on every
boot. Nothing in the build disables ADWS, so the original disable came from
outside the current tree (older build or manual); the self-heal makes the source
moot.

### Added — 2026-09-07 (Test-RangeConfig.ps1 -Next — pipeline navigation)
`Test-RangeConfig.ps1` gains a **`-Next`** switch that prints the single command to
run next in the Stage → Setup → Test → Reset pipeline. It reads the `STAGED.txt` /
`READY.txt` markers in `C:\ProgramData\CyberRange` plus this run's own results and
recommends: build (Stage), apply misconfigs (Setup), fix drift (`-Repair`), bring
ADWS up first when AD DS is unreachable (so the operator isn't sent to `-Repair`,
which can't help there), or tear down (Reset) once 0 FAIL. Read-only — it only
reads markers, never creates them.

### Changed — 2026-09-07b (restructure: three steps, and the build order reversed)

The nine top-level scripts are now **three**, matching the three things an
operator actually does. The build order changed with them, and that is the
important part.
...
| | Was | Now |
|---|---|---|
| Build the domain | `Stage` + `Setup` phases 1-2 + `Resume` | **`Stage-CyberRange.ps1`** |
| Break it | `Setup` phases 1 and 3, + 11 category scripts | **`Setup-CyberRange.ps1`** |
| Check it | `Test-RangeConfig` + `Repair-RangeConfig` | **`Test-RangeConfig.ps1`** (`-Repair`) |
| Undo it | `Reset-CyberRange.ps1` | unchanged, kept separate |

Each step prompts to continue into the next; `-Auto` chains all three unattended.

#### The build order is reversed, on purpose

Previously the box was weakened **first** (phase 1) and promoted **second** —
promotion had to survive a host with no firewall, no UAC and a downgraded LSA.
That is fragile, and it is where the field lockouts came from: the dangerous
window was "weakened, rebooting, no domain account yet".

Now `Stage` builds a small, ordinary, **working** domain — DC promoted, OU tree,
staff and service accounts, operator account provisioned and authenticated before
anything else — and `Setup` breaks it afterwards. If Setup goes wrong you still
have a healthy DC and a working login. Two consequences fall out for free:

- `20-credential-exposure`'s autologon block no longer has to run twice (once
  standalone, once after promotion to fix `DefaultDomainName`). It runs once,
  after promotion, in the correct domain form.
- The four-phase state machine collapses. Stage keeps a resume task because
  promotion reboots; Setup is a single pass.

#### Users: created ordinary, weaponised separately

`scripts\directory.ps1` (Stage) creates the OU tree and the eight staff/service
accounts as **ordinary** accounts. `scripts\ad-users-weaponise.ps1` (Setup) then
stamps the attack attributes onto a named subset: SPNs + RC4 for Kerberoast,
`DoesNotRequirePreAuth` for AS-REP, reversible encryption, password-in-description.
The vulnerable accounts are now drawn from the normal population rather than
being a separate, obviously-planted set.

#### Removed

- **The clone/golden-image path.** `Initialize-RangeClone.ps1`,
  `Get-RangeCloneSecret.ps1`, `Get-RangeCloneIdentity` and `-Mode Image` are gone.
  Each VM is now built from scratch on fresh Server 2025, which closes **F16** and
  **F17** by construction — there is no shared `krbtgt` or domain SID because
  there is no clone.
- **`Resume-CyberRange.ps1`** — Stage resumes itself; re-running it is the manual path.
- **`Repair-RangeConfig.ps1`** — folded into `Test-RangeConfig.ps1 -Repair`, which
  keeps the same test → fix → re-test engine and the DC-GPO delegated fixers.
- **Eleven machine-level category scripts** (`05`, `10`–`90`, `75`). Since the
  control table landed they were thin wrappers; their imperative prerequisites
  (the FS-SMB1 feature install, `Enable-PSRemoting`, the beacon payloads, the
  autologon block) now live inline in Setup where the order is visible.

#### Renamed — no more numeric prefixes

The remaining helpers say what they are: `preflight.ps1`, `operator-account.ps1`,
`operator-keeper.ps1`, `directory.ps1`, `ad-users-weaponise.ps1`,
`ad-certificates-esc1.ps1`, `ad-acl-delegation.ps1`, `ad-gpo-legacy.ps1`,
`ad-hidden-admins.ps1`, `dc-security-gpo.ps1`. The `scripts\dc\` directory is
gone. **21 files under `scripts\` became 10.**

#### Credentials

`DC.SafeModePassword` and `Analyst.Password` were still the shipped example
values (`Password123!`, `bb123#123`); both are now 24-character random secrets.
`LocalAdminAutoLogonPass` and `HiddenAdminPassword` are deliberately unchanged —
the first is the exposed-credential lesson, the second is loot that has to stay
crackable.

### Changed — 2026-09-07 (de-duplication: one control table, one recovery account, one install command)

The build worked; it was just written down more times than necessary. Nothing
here changes what the range *does* to a host — the same controls are applied with
the same values. What changed is how many places say so.

#### One control table (`modules/RangeControls.psm1`) — NEW

Every intentional misconfiguration used to be written out **four times**: the
category script applied it, `Test-RangeConfig.ps1` checked it, `Repair-RangeConfig.ps1`
re-tested *and* re-fixed it, and `Reset-CyberRange.ps1` reverted it. Twenty-six
registry value names appeared verbatim in three of those four. Nothing kept the
copies in agreement, so a control could be applied one way and checked another —
and comments had to be written explaining which copy won.

- **Added `modules/RangeControls.psm1`** — 125 controls, each declared once with
  its intended value, NIST/CIS/CVE mapping, host-role and config gates, how to
  check it (registry compare, or a live probe of the running subsystem), how to
  revert it, and the 2025-vs-2019 rationale that used to live as scattered code
  comments. Four verbs act on that table: `Set-RangeControl` (apply),
  `Test-RangeControl` (check), `Reset-RangeControl` (revert), plus
  `Invoke-RangeControlCategory` for the category scripts.
- **`scripts/10,20,30,40,50,60,70,75,80`** now apply from the table. What stays in
  each script is only what a table cannot express: the FS-SMB1 feature install
  that must precede the SMB1 controls, `Enable-PSRemoting` before the WSMan
  settings, the beacon payload generation, and the autologon block in
  `20-credential-exposure.ps1` (it sets a real account's password and decides from
  the result whether enabling autologon is safe — F1).
- **`Test-RangeConfig.ps1` 817 → 388 lines.** The machine-level sections are now
  one loop over the table. What remains hand-written is what the table does not
  describe: the answer-key ACL, the operator account, the hidden admins, and the
  AD scenarios seeded by `scripts/dc/`.
- **`Repair-RangeConfig.ps1` 394 → 257 lines.** Same test→fix→re-test engine, now
  over the table instead of 22 hand-written entries. Controls that cannot be
  repaired on a live host carry a `NotRepairable` reason in the table and are
  reported N-A with it, never "fixed".
- **`Reset-CyberRange.ps1` 515 → 374 lines.** Reverts from the table. The
  teardown's own category names are kept (they are the `-Only` vocabulary) and
  map to table categories. The firewall still reverts **last** in the network
  block so a remote operator is not cut off mid-teardown.
- **Net effect:** adding or changing a control is one edit in one file. It is no
  longer possible for the build and the checker to disagree about what a control
  is, because there is only one definition.

#### One recovery account

- **Removed the `rangebreak` account and the `BreakGlass` config block.** It was
  `analyst` built a second time: same role-aware local-or-domain provisioning,
  same admin groups, same un-hiding from the sign-in screen, same authentication
  check — reading from the same config file, so it shared every failure mode with
  the account it was supposed to back up. `analyst` is strictly better because
  `02-operator-keeper.ps1` re-asserts it on **every boot**.
- `00-break-glass.ps1` **-Apply now calls `01-analyst-admin.ps1`** instead of
  carrying a second copy of the provisioning logic (~120 lines removed).
- `Setup-CyberRange.ps1`: `Invoke-BreakGlass` and `Invoke-Analyst` were ~40
  near-identical lines each; they are now one `Invoke-OperatorAdmin`.
- **The recovery layers are unchanged and are what actually matter.**
  `docs/BREAK-GLASS.md` now documents seven, and gained the one that was missing
  from the runbook entirely: the **logon-screen SYSTEM shell** (Win+U / Shift ×5
  → `C:\range-fix.cmd`), which the build has armed since Phase 1 all along.
- **Backward compatible:** a VM built before today still has `rangebreak`;
  teardown still removes it, and `Get-RangeCloneSecret.ps1` still prints its
  derived password (marked legacy). `Get-RangeCloneIdentity` keeps the
  `breakglass` derivation for exactly that reason.

#### One install command

- **`Stage-CyberRange.ps1` now runs the break-glass pre-flight itself** and
  refuses to hand off to the engine if a blocking gate fails — while the box is
  still healthy, which was always the stated reason for running it first. The
  documented procedure was: edit config → run the pre-flight → run it `-Apply` →
  re-run until PASS → stage. It is now: edit config → stage. Added
  `-SkipPreflight` for the operator who has just run it by hand.

### Fixed — 2026-09-07

- **`-Mode Image` was broken.** `Setup-CyberRange.ps1` used `$Mode` (to stop at
  the end of Phase 1 for a golden image) but never declared it as a parameter,
  while `Stage-CyberRange.ps1` passed `-Mode Image`. Under `[CmdletBinding()]`
  that is a parameter-binding error, so **the entire F16-safe class-build path
  would fail at launch**. `-Mode` is now declared with the same ValidateSet.
- **The placeholder-credential gate (F20) was checking values the repo does not
  ship.** `Assert-RangeSafety` looked for `'Password!'`, `'S@feM0de-ChangeMe!'`
  and `'ChangeMe-BreakGlass!2026'`; the config actually ships `'Password123!'` and
  `'bb123#123'`. The gate therefore passed on a completely untouched config and
  protected nothing. The list now matches what ships, and a comment says to keep
  the two in sync. **This is a behaviour change:** a config still carrying the
  shipped `DC.SafeModePassword` or `Analyst.Password` will now be refused (use
  `-Force` for a deliberate off-network dry run).

### Changed — 2026-09-07 (docs)

- **`README.md` rewritten.** It still carried "Do not clone this image yet" and
  told the reader the clone model had a critical unfixed flaw (F16) — which
  `docs/SETUP-GUIDE.md` had already documented as *implemented*. That
  contradiction was the first thing a new reader hit. Also fixed: a pointer to
  `scripts\dc-ad-users-roast.ps1` (no such file), `scripts\dc\00..50` (there is a
  `60`), and a category table missing `dc\50` and `dc\60`.
- **Archived to `docs/archive/`** (moved, not deleted — this repo is not under
  version control): `IMAGE-DESIGN-PROMPTS.md` (a prompt library for a design
  session, never operator documentation; the topology question it exists to ask
  was settled 2026-09-03) and `VERIFICATION-REPORT-2026-09-03.md` (a superseded
  snapshot whose verdict was "DO NOT BUILD"). Links updated across the live docs;
  `docs/archive/README.md` says why each is there.
- **`docs/IMPROVEMENT-PLAN.md`** said "Nothing here has been implemented" while
  WP2 was in the tree. It now carries a status block naming what has landed.
